...
| Spintronics chain speed/direction | speed = k*I, sign exact; on-screen link motion amplified | exact mapping, visualization speed | Spintronics projection | low |
| Spintronics tension colour | same node potentials as Potential layer | approx (same model) | Spintronics projection | medium |
| Spintronics spring compression | displacement proportional to solved Vc | exact mapping, symbolic geometry | Spintronics projection | low |
| Spintronics flywheel spin | angular momentum proportional to solved Il; spoke phase animated | exact mapping, visualization speed | Spintronics projection | low |
| Spintronics brake heat | dissipated power only (same as Heat) | exact-sign | Spintronics projection | low |

## Notes

- Solver branch values remain the source of truth for current, voltage drop, and signed power. All projections (Schematic, Physics, Spintronics) are built by `ProjectionBuilder` from the same model + solution; no projection invents physics.
...
- Drift particles and spintronics chain/flywheel motion are visual timing aids: directions and relative magnitudes are solver-honest, animation speed is amplified for visibility (`kVisual*` constants in `ProjectionBuilder.cpp`).
...
- The spintronics projection preserves power correspondence exactly: tension x speed = V x I (unit constants multiply to 1, see `SpintronicsMapping.h`).
