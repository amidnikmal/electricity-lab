...
- **PumpAloneDrivesTheLoop красный**: assist выключен (targetSpeed=0 во
  всех спеках), крыльчатка крутится — своя труба течёт по току (srcFlow>0
  ПРОХОДИТ), но циркуляция НЕ доходит до >=2 соседних труб (порог 0.05
  wu/s за 5 с). Уже пробовано: packing 0.50->0.62, damping 0.25->0.10,
  лопасти толще (0.6r). Гипотезы: (а) пустые камеры узлов = слак, который
  съедает продвижение фронта — посеять частицы и В КАМЕРАХ; (б) обратная
  утечка над лопастями (коридор +0.45hw..+hw) — сузить до 0.3hw или
  добавить статический язык-отсекатель у выхода насоса; (в) просто мало
  времени/скорости — поднять ω клампы, удлинить тест до 8-10 с, снизить
  порог. Отлаживать: печатай means по компонентам прямо в тесте.
  Пока тест можно подержать DISABLED_ (так и закоммичено), но цель —
  зелёный без disable.
- Визуально не проверено в приложении (запустить, Hydraulic-вид, смотреть
  плотность/переток/что насос гонит).
