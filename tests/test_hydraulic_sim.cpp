// Тесты ВАРИАНТА Б гидравлики (HydraulicSim — карантин, в приложение не
// подключён; статус и критерии удаления — в src/physics/HydraulicSim.h).
// Они здесь, чтобы запасная реализация не сгнила: если этот файл начал
// падать или мешать — это сигнал пересмотреть, нужен ли ещё вариант Б.
